...
const ActivityDetailsChat = observer(function ActivityDetailsChat() {
  ...
  const addComment = async (data: FieldValues) => {
    try {
      // The name of the invoked method must match exactly the name of the method as defined in the CommentHub.cs file
  ...
  const handleKeyPress = (event: React.KeyboardEvent<HTMLDivElement>) => {
    if (event.key === 'Enter' && !event.shiftKey) {
      ...
      handleSubmit(addComment)(); // We need to execute the handleSubmit here and not just call it. So we add the () at the end.
    }
  };
