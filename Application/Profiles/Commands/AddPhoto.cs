...
public class AddPhoto
{
    ...
    public class Handler(IUserAccessor userAccessor, AppDbContext dbContext, IPhotoService photoService) : IRequestHandler<Command, Result<Photo>>
    {
        public async Task<Result<Photo>> Handle(Command request, CancellationToken cancellationToken)
        {
            ...
            user.ImageUrl ??= photo.Url; // This expression checks if the left operand is null and if so, assigns it the right operand

            dbContext.Photos.Add(photo);
            var result = await dbContext.SaveChangesAsync(cancellationToken) > 0;
            return result ? Result<Photo>.Success(photo) : Result<Photo>.Failure("Problem saving photo to the DB", 400);
        }
    }
}
