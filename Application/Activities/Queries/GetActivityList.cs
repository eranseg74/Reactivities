...
public class GetActivityList
{
    ...
    public class Handler(AppDbContext context/*, ILogger<GetActivityList> logger*/, IMapper mapper, IUserAccessor userAccessor) : IRequestHandler<Query, Result<List<ActivityDto>>>
    {
        ...
        public async Task<Result<List<ActivityDto>>> Handle(Query request, CancellationToken cancellationToken)
        {
            ...
            // }
            var result = await context.Activities.ProjectTo<ActivityDto>(mapper.ConfigurationProvider, new { currentUserId = userAccessor.GetUserId() }).ToListAsync(cancellationToken);
            if (result == null)
            {
