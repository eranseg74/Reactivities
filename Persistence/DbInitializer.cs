...
public class DbInitializer
{
    public static async Task SeedData(AppDbContext context, UserManager<User> userManager)
    {
        ...
        if (!userManager.Users.Any())
        {
            ...
            {
                await userManager.CreateAsync(user, "Pa$$w0rd"); // This adds the user to the DB. No need for SaveAsync
            }
        }
        ...
        // No need to await as AddRange because here we are not dealing with async operation. We are just tracking entities in memory.
        context.Activities.AddRange(activities);
        await context.SaveChangesAsync(); // This is the async operation that saves changes to the database.
    }
}
